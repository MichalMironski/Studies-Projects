...
void Translation(int choice, string FirstLang, string SecondLang, string FileName, string beforeTranslation)
{
	...
	if (!isWord(beforeTranslation))
		cout << beforeTranslation << " - to nie wyglada na slowo. Slowo powinno skladac sie z samych liter." << endl << endl;
	else
	{
		...
		if (found == 1)
		{
			if (line.substr(0, spacerPos) != toLowerCase(beforeTranslation) &&
				line.substr((spacerPos + 1), (line.length() - spacerPos)) != toLowerCase(beforeTranslation))
			{
			...
			else
			{
				...
				if (afterTranslation == toLowerCase(beforeTranslation))
				{
                    cout << FirstLang << beforeTranslation << endl;
					cout << "Slowo jest juz przetlumaczone." << endl << endl;
				}
				else
				{
					cout << FirstLang << beforeTranslation << endl;
...
//Function add new translation to dictionary - DONE
